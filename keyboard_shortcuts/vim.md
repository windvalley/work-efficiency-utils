...
Go:
    g d: 跳转到光标所在函数或package的定义位置, 水平分屏的方式展示
    ...
    ,r: 运行当前Go脚本
    ,t: go test测试当前的Go文件
    ,s: 查看光标所在的type或method是否正确, 注意只能用来检查type或者method, 函数等其他不行

Python:
    ,r: 运行当前Python脚本
    ,b: 添加或删除断点, 添加完成后需要在命令行来执行该python脚本进行断点测试
    :Black: 格式化当前Python整个文件代码, 已配置为保存时自动格式化
```
