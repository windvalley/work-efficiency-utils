...
```
通用:
    shift k/K: 查看光标所在模块或函数的帮助文档
    g d: 前提是在同一个文件内, 跳转到光标所在函数的定义位置
    g f: 直接跳转到光标所在的文件名所对应的文件中
    ctrl w f: 水平分屏展示光标所在的文件名所对应的文件
    ctrl k/j: 向上或向下跳转到下一个错误提示的位置

Go:
    g d: 跳转到光标所在函数或package的定义位置, 以水平分屏的方式展示
    ...
    ,r: 运行当前Go脚本
    ,t: go test测试当前的Go文件
    ,s: 检查光标所在的type或method是否正确, 注意只能用来检查type或者method, 函数等其他对象则不行

Python:
    ,r: 运行当前Python脚本
    ,b: 添加或删除断点, 添加完成后需要在命令行来执行该Python脚本进行断点测试, 不能使用`,r`来运行
    :Black: 格式化当前Python整个文件代码, 已配置为保存时自动格式化代码
    :Isort: 自动调整import的顺序
    ctrl i: 在VISUAL BLOCK模式下选择待调整的import区域后, 再按该快捷键即可完成自动调整
